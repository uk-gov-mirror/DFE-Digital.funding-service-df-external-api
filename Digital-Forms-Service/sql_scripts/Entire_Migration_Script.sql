		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				DECLARE @FormId nvarchar(250), @FormStatus nvarchar(250), @Createdbyid nvarchar(250),
					...
					@csv nvarchar(250), @selectedDocument nvarchar(250), @tabchildetail nvarchar(250), @tabName nvarchar(250)
				DECLARE @status bit
				...
				DECLARE @pagedata nvarchar(max), @pagechildrendata nvarchar(max), @componentdata nvarchar(max), 
					...
					@tabsjson nvarchar(max), @outputdata nvarchar(max), @outputdetailskey nvarchar(max), @outputdetailsvalue nvarchar(max),
                  ...
                  WHILE @@FETCH_STATUS = 0
					...
					--7. DesignDataSet Table
								...
								SELECT
									...
									, JSON_VALUE(@designDS, '$.keyIdentifier') AS KeyIdentifier
												...
												WHILE @@FETCH_STATUS = 0
													BEGIN
														...
														SELECT
															...
															, JSON_VALUE(@DDSDetailsRow, '$.value') as [Value]
															, CASE WHEN
				...
				-- PAGES LOOP
					...
					--11. Components Table
												...
												----if @componentSettings!=0
													...
													DECLARE compadditionalsettingsjson CURSOR FOR SELECT [key], [value], [type]
																																				FROM OPENJSON(@componentdata) AS json 
																																				WHERE [key] NOT IN ('type', 'isEditingTabs', 'title', 'hint', 'name', 
																																				'nameHasError', 'componentEdited', 'content', 'checked')
					...
					--13. TabDetails table
																				...
																				-- Get the Id for the TabDetails just created above
																					...
																					-- Open a cursor on the tabsData object for the current component
																					DECLARE tabsjson CURSOR FOR SELECT [key], [value]
																																			FROM OPENJSON(JSON_QUERY(@componentvalue, '$.')) AS JSON
																					OPEN tabsjson
					...
					--14. TabChildDetails Table
																									...
																									SELECT
																											@CADSTID as CADSTID
																										, @TABID as TABID
																										, JSON_VALUE(@tabsjson, '$.tabLabel') as tabLabel
																										, JSON_VALUE(@tabsjson, '$.tabHeader') as tabHeader
																										, JSON_VALUE(@tabsjson, '$.type') as [type]
																										, JSON_VALUE(@tabsjson, '$.value') as [Value]
					...
					--19. CalculationComponentDetails table
											...
											SELECT
												@CALCID as CALCID
												, (SELECT CMPID FROM [Components] WHERE Name=JSON_VALUE(@calculationdata, '$.name') 
												AND PGID IN (SELECT PGID FROM Pages WHERE Title=JSON_VALUE(@calculationdata, '$.pageLocation')))
															...
															BEGIN
																IF @subconditionkey='field' or  @subconditionkey='value'
																	...
																	SELECT
																		@CNID as CNID
																		, @SUBID as SubsetNo
																		, JSON.[value] as PropName
																		, JSON.[key] as PropValue
